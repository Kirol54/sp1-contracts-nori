...
contract PlonkVerifier {
    ...
    uint256 private constant VK_QL_COM_X =
        17984004271810887111892332882651374483612811613160171882231530673955342124072;
    uint256 private constant VK_QL_COM_Y =
        13277994607875064672295433418223618455646015691912141325106751561841646085822;
    uint256 private constant VK_QR_COM_X =
        16336286973672788525637509231329701034039020185740400021577697914995487271901;
    uint256 private constant VK_QR_COM_Y =
        88284377293612431075210969180485407669835075356048788125030179865751136579;
    uint256 private constant VK_QM_COM_X =
        10856514239855788242760873537198968126773773371401212930763195728908952457266;
    uint256 private constant VK_QM_COM_Y =
        9275209459688740642101488227886989724826404626746083670876354123229317070279;
    uint256 private constant VK_QO_COM_X =
        11483266664351462882096699260743761962335837409017508503826409502538996076321;
    uint256 private constant VK_QO_COM_Y =
        17394959804451514544078291653193154652690985857122468223674247897665667008225;
    uint256 private constant VK_QK_COM_X =
        6378083169737830823085597784511445554718388062388953364429190548392020833332;
    uint256 private constant VK_QK_COM_Y =
        12423874058816725405347015291038805106767729770869142043443023930694783565135;

    uint256 private constant VK_S1_COM_X =
        8432363989348148399267595343996871949745756313266171098803972147558566645391;
    uint256 private constant VK_S1_COM_Y =
        21830041218908693046173167152196424202345484683169853420596524951634570306585;

    uint256 private constant VK_S2_COM_X =
        2889872191603241225798623124219684248629415321428218534303823116357979417612;
    uint256 private constant VK_S2_COM_Y =
        8446598484540146291961338772594737249603462456099116591415979984815127389111;

    uint256 private constant VK_S3_COM_X =
        12894021857927799668653295424700631150139887020229360316861211114803005826079;
    uint256 private constant VK_S3_COM_Y =
        5332155988317853016248818837542073627058808403298822656476772250044723938116;

    uint256 private constant VK_COSET_SHIFT = 5;

    uint256 private constant VK_QCP_0_X =
        16113316688614670925300831064391815772431716488200414163545127725991745431527;
    uint256 private constant VK_QCP_0_Y =
        18613967203673816695007264416375047840688367875268537453854528850830700989951;
    ...
    // -------- utils (for hash_fr)
    uint256 private constant HASH_FR_BB =
    ...
    function Verify(
        ...
        assembly {
            ...
            prev_challenge_non_reduced := derive_gamma(
                proof.offset,
                public_inputs.length,
                public_inputs.offset
            )
            prev_challenge_non_reduced := derive_beta(
                prev_challenge_non_reduced
            )
            prev_challenge_non_reduced := derive_alpha(
                proof.offset,
                prev_challenge_non_reduced
            )
            ...
            let zeta_power_n_minus_one := addmod(
                pow(zeta, VK_DOMAIN_SIZE, freeMem),
                sub(R_MOD, 1),
                R_MOD
            )
            mstore(
                add(mem, STATE_ZETA_POWER_N_MINUS_ONE),
                zeta_power_n_minus_one
            )

            // public inputs contribution
            let l_pi := sum_pi_wo_api_commit(
                public_inputs.offset,
                public_inputs.length,
                freeMem
            )
            let l_pi_commit := sum_pi_commit(
                proof.offset,
                public_inputs.length,
                freeMem
            )
            ...
            function check_proof_size(actual_proof_size) {
                let expected_proof_size := add(
                    FIXED_PROOF_SIZE,
                    mul(VK_NB_CUSTOM_GATES, 0x60)
                )
            ...
            function derive_alpha(aproof, beta_not_reduced)
                -> alpha_not_reduced
            {
                let state := mload(0x40)
                let mPtr := add(mload(0x40), STATE_LAST_MEM)
                ...
                calldatacopy(
                    _mPtr,
                    add(aproof, PROOF_GRAND_PRODUCT_COMMITMENT_X),
                    0x40
                )
                let l_success := staticcall(
                    gas(),
            ...
            function batch_compute_lagranges_at_z(z, zpnmo, n_pub, mPtr) {
                ...
                for {
                    ...
                    mstore(
                        _mPtr,
                        mulmod(mulmod(mload(_mPtr), zn, R_MOD), _w, R_MOD)
                    )
                    _mPtr := add(_mPtr, 0x20)
                    _w := mulmod(_w, VK_OMEGA, R_MOD)
                }
            ...
            function sum_pi_commit(aproof, nb_public_inputs, mPtr)
                -> pi_commit
            {
                let state := mload(0x40)
                let z := mload(add(state, STATE_ZETA))
                ...
                pi_commit := compute_ith_lagrange_at_z(
                    z,
                    zpnmo,
                    add(nb_public_inputs, VK_INDEX_COMMIT_API_0),
                    mPtr
                )
            }
            ...
            function compute_ith_lagrange_at_z(z, zpnmo, i, mPtr) -> w {
                w := pow(VK_OMEGA, i, mPtr) // w**i // omega^i
            ...
            function batch_verify_multi_points(aproof) {
                ...
                mstore(
                    add(mPtr, 0x20),
                    mload(add(state, STATE_FOLDED_DIGESTS_Y))
                )
                mstore(
                    add(mPtr, 0x40),
                    calldataload(add(aproof, PROOF_BATCH_OPENING_AT_ZETA_X))
                )
                mstore(
                    add(mPtr, 0x60),
                ...
                mstore(
                    add(mPtr, 0x80),
                    calldataload(add(aproof, PROOF_GRAND_PRODUCT_COMMITMENT_X))
                )
                mstore(
                    add(mPtr, 0xa0),
                    calldataload(add(aproof, PROOF_GRAND_PRODUCT_COMMITMENT_Y))
                )
                mstore(
                    add(mPtr, 0xc0),
                    calldataload(add(aproof, PROOF_OPENING_AT_ZETA_OMEGA_X))
                )
                mstore(
                    add(mPtr, 0xe0),
                    calldataload(add(aproof, PROOF_OPENING_AT_ZETA_OMEGA_Y))
                )
                ...
                mstore(
                    ...
                    sub(P_MOD, mload(folded_evals_commit_y))
                )
                point_add(
                    folded_digests,
                    folded_digests,
            ...
            function fold_state(aproof) {
                ...
                point_acc_mul_calldata(
                    state_folded_digests,
                    add(aproof, PROOF_L_COM_X),
                    acc_gamma,
                    mPtr
                )
                fr_acc_mul_calldata(
                    add(state, STATE_FOLDED_CLAIMED_VALUES),
                    add(aproof, PROOF_L_AT_ZETA),
                    acc_gamma
                )
                ...
                point_acc_mul_calldata(
                    state_folded_digests,
                    add(aproof, PROOF_R_COM_X),
                    acc_gamma,
                    mPtr
                )
                fr_acc_mul_calldata(
                    add(state, STATE_FOLDED_CLAIMED_VALUES),
                    add(aproof, PROOF_R_AT_ZETA),
                    acc_gamma
                )
                ...
                point_acc_mul_calldata(
                    state_folded_digests,
                    add(aproof, PROOF_O_COM_X),
                    acc_gamma,
                    mPtr
                )
                fr_acc_mul_calldata(
                    add(state, STATE_FOLDED_CLAIMED_VALUES),
                    add(aproof, PROOF_O_AT_ZETA),
                    acc_gamma
                )
            ...
            function compute_gamma_kzg(aproof) {
                ...
                mstore(
                    add(state, STATE_GAMMA_KZG),
                    mod(mload(add(state, STATE_GAMMA_KZG)), R_MOD)
                )
            }

            function compute_commitment_linearised_polynomial_ec(
                aproof,
                s1,
                s2
            ) {
                ...
                for {
                    ...
                    mstore(
                        add(mPtr, 0x20),
                        calldataload(add(bsb_commitments, 0x20))
                    )
                    point_acc_mul(
                        add(state, STATE_LINEARISED_POLYNOMIAL_X),
                        mPtr,
            ...
            function point_acc_mul(dst, src, s, mPtr) {
                ...
                l_success := and(
                    l_success,
                    staticcall(gas(), EC_ADD, mPtr, 0x80, dst, 0x40)
                )
                if iszero(l_success) {
                    error_ec_op()
                }
